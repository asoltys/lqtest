...
function rangeproofPubExpand(pubs, exp, rsizes, rings, genp) {
  var base = ProjectivePoint.fromHex(genp);
  var i, j, npub;
  if (exp < 0) {
    exp = 0;
  }
...
async function rangeproofSign(
  ...
  let genP = ProjectivePoint.fromHex(genp);
  console.log("hex", genp);
  console.log(parseGen(genp));
  console.log("gen", b2h(serializePoint(genP)));
  ...
  console.log("stmp", b2h(n2b(stmp)));
  console.log("sec", b2h(n2b(sec[rings - 1])));
  sec[rings - 1] = (sec[rings - 1] + stmp) % CURVE.n;
  console.log("aftN", sec[rings - 1]);
  console.log("aft", b2h(n2b(sec[rings - 1])));
